...
// without touching the CouchDB index (no InitFs: the index is shared with
// the source and must not be reinitialized). S3 buckets are initialized at
// startup; Swift target containers are created here.
func buildTarget(inst *instance.Instance, to string) (vfs.VFS, vfs.Avatarer, error) {
	...
	case config.SchemeS3:
		...
		dstAv := vfss3.NewAvatarFs(storage.Client, storage.Bucket, storage.Prefix+inst.DBPrefix()+"/")
...
func purgeSource(inst *instance.Instance, srcScheme string) error {
	...
	case config.SchemeS3:
		storage := config.GetS3Storage(config.S3StorageFiles)
		prefix := storage.Prefix + inst.DBPrefix() + "/"
		return s3util.DeletePrefixObjects(context.Background(), storage.Client, storage.Bucket, prefix)
